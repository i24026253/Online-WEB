...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_course'])) {
    $course_id = (int)$_POST['course_id'];
    $update_query = "UPDATE dbo.Courses SET IsActive = 0, UpdatedDate = ? WHERE CourseID = ?";
    $update_params = array(date('Y-m-d H:i:s'), $course_id);
    $update_result = sqlsrv_query($conn, $update_query, $update_params);

    if ($update_result) {
        $message = "<p style='color:green;'>✅ Course deleted successfully.</p>";
    } else {
        error_log("Delete course error: " . print_r(sqlsrv_errors(), true));
        $message = "<p style='color:red;'>❌ Failed to delete course: " . print_r(sqlsrv_errors(), true) . "</p>";
    }
}
...
// Fetch active courses with lecturer assignments
$courses_query = "
                    ...
                    <table class="table table-hover">
                        ...
                        <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <form method="POST" action="courses.php?username=<?php echo urlencode($username); ?>" style="display:inline;">
                                            ...
                                            <button type="submit" name="delete_course" class="btn btn-outline-danger btn-sm" onclick="return confirm('Are you sure you want to delete this course?');">
                                ...
                                <?php if (!$course['FirstName']) { ?>
                                    <div class="modal fade" id="assignModal<?php echo $course['CourseID']; ?>" tabindex="-1" aria-labelledby="assignModalLabel<?php echo $course['CourseID']; ?>" aria-hidden="true">
                                        <div class="modal-dialog">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h5 class="modal-title" id="assignModalLabel<?php echo $course['CourseID']; ?>">Assign Lecturer to <?php echo htmlspecialchars($course['CourseCode']); ?></h5>
                                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                                </div>
                                                <form method="POST" action="courses.php?username=<?php echo urlencode($username); ?>">
                                                    <div class="modal-body">
                                                    ...
                                                    <div class="modal-footer">
                                                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                                                        <button type="submit" name="assign_lecturer" class="btn btn-primary">Assign Lecturer</button>
                                                    </div>
                                                </form>
                                            </div>
                                        </div>
                            ...
                            <?php if (sqlsrv_num_rows($courses_result) === 0) { ?>
                                <tr><td colspan="8" class="text-center text-muted">No active courses found.</td></tr>
                            <?php } ?>
                        </tbody>
                    </table>
                </div>
